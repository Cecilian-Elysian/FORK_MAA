...
| 分支 | HEAD | 状态 | 关键 commit | 说明 |
|------|------|------|-------------|------|
| `feat/expedite-threshold` | 待 commit | 实施中 | 待生成 | 公招加急门槛：加急许可仅在确认招募组合的最低星级 ≥ 用户设定阈值（0=全部 / 4 / 5 / 6）时使用；3★ 不浪费加急许可换不到黄票；默认 0 保持向后兼容 |

详见 `LOG.md`「2026-07-23 feat/expedite-threshold 启动」。


## 7. 分支生命周期记录

以下分支已完成使命，已于 2026-07-16 删除（本地删，远端保留）。
...
| 生命周期 | 2026-07-15 创建 → 2026-07-23 删除（未合入） |
| 关键 commit | `3529ab0f05` |
| 详见 | `LOG.md` 2026-07-15 |
