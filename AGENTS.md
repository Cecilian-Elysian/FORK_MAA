...
| `fix/recruit-screenshot-monitor-channels` | 修多 slot 公招 hire_all 在第 2 个 slot 触发 MAA 进程崩溃：根因 `RecruitScreenshotMonitor::check` 的 `cv::absdiff(gray, m_last_frame, diff)` 第一次调用存 `image.copyTo`（BGR 3 通道），第二次调用 `gray` 是 GRAY 1 通道，channels 不一致 → `cv::arithm_op` 抛 `cv::Exception` → OpenCV 默认 `CV_ERROR_ABORT` → `SIGABRT` → 进程崩溃。修复：(1) `m_last_frame` 改存 `gray`（单通道），与 absdiff 输入 channels 一致；(2) absdiff 条件增加 `m_last_frame.type() == gray.type()` 防御性检查。**仅本仓库 fork 私有代码**（upstream/dev-v2 没有 `RecruitScreenshotMonitor`，其 `hire_all` 也不调 monitor，所以上游不暴露这个 bug） | `staging`（修复 `branch` 自身，按 §3.3 合并目标为 staging），关键 commit `84b339a870`，详见 `LOG.md` 2026-07-30 |
| `fix/auto-recruit-expedite-original-level` | 修 `feat/auto-recruit-3star-to-4star` 语义冲突：3★ 组合里有 4★ 潜力时 `m_last_confirmed_min_level` 被升级为 4，与 `feat/expedite-threshold` 的加急门槛 `m_expedite_min_level >= 4` 冲突，导致非必出 4★ 的场景误用加急许可。修复：新增 `m_original_min_level` 追踪升级前原始最低星级，加急判定改用 `m_original_min_level`（通过扫描 `final_combination.opers` 中 ≥3★ 的最低干员等级推导，仅当全部 opers ≥4★ 时才为 4，即「必出四星才加急」） | `staging`，关键 commit `dddc4d64f2`，详见 `LOG.md` 2026-07-31 |
