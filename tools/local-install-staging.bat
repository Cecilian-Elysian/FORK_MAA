...
> ".\global.json" echo {"sdk":{"version":"10.0.100","rollForward":"latestFeature"}} || goto :error
...
cmake --install build --config RelWithDebInfo --prefix install-staging || goto :error

dotnet restore src/MaaWpfGui/MaaWpfGui.csproj || goto :error

dotnet publish src/MaaWpfGui/MaaWpfGui.csproj -c Release -r win-x64 -o install-staging /p:DisableBeauty=True || goto :error
"%netbeauty_bin%" --usepatch "%CD%\install-staging\." ./externals || goto :error

del /f .\install-staging\*.h 2>nul
rmdir /s /q .\install-staging\msvc-debug 2>nul
